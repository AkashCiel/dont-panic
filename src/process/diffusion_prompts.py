"""Prompt builders for the diffusion track (framework: framework/ai_diffusion_framework.md)."""
import json
import os
...
def build_diffusion_wave1_system_prompt() -> str:
    ...
    return f"""You are running Wave 1 of the AI Diffusion Tracker. Follow the framework below exactly.
...
If required context is missing per the framework, respond ONLY with a JSON object: {{"error": "MISSING CONTEXT", "detail": "<why>"}}."""
...
def build_diffusion_wave2_system_prompt() -> str:
    fw = get_diffusion_framework_text()
    return f"""You are running Wave 2 synthesis for the AI Diffusion Tracker. Follow the framework below.

{fw}

## Wave 2 instructions

Aggregate Wave 1 findings into one coherent diffusion report tree JSON. Use the cognitive context block in the user message when present; if it is absent or explicitly states no report is available, follow the MISSING CONTEXT rules for Q2 and scenario-dependent claims.

Respond with JSON only (no markdown fences). Use the schema given in the user message."""
...
Output a single JSON object only (no markdown). Top-level shape:
{{
  "executive_summary": {{
    "id": "exec",
...
Include 7 Layer-1 themes from the framework (executive summary plus six thematic sections) — you may combine or split using nested `sections` as long as coverage is complete."""
...
def build_diffusion_wave2_partial_prompt(
    ...
    """Intermediate call when splitting by cascade order."""
...
def build_diffusion_wave2_merge_prompt(
    ...
    """Final merge after cascade splits."""
    return f"""{cognitive_context_block}
...
Merge into the final diffusion report tree JSON (same schema as the single-call Wave 2 user prompt). Output JSON only."""
